...
declare
  owner_id uuid;
  claim_id constant text := 'personal-owner-grant-2026-08-04-second';
begin
  ...
  values (
    owner_id,
    jsonb_build_object('tokens', 640, 'rewardClaims', jsonb_build_array(claim_id)),
    now()
  )
  ...
  set state = case
      when coalesce(public.user_economy.state -> 'rewardClaims', '[]'::jsonb)
        ...
        jsonb_set(
          ...
          '{tokens}',
          to_jsonb(coalesce((public.user_economy.state ->> 'tokens')::integer, 240) + 400),
          true
        ),
        '{rewardClaims}',
